...
export function EventScreen({ initialCode }: EventScreenProps) {
  ...
  const animateIn = useCallback(
    (state: AppState, el: HTMLDivElement | null, tl: gsap.core.Timeline) => {
      switch (state) {
        case AppState.Idle:
  ...
  const startAttendance = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        if (!result.success) {
          throw new Error(
            result.message || result.error || "Attendance failed"
          );
        }

        toast.success("Attendance recorded", {
          description: `${result.student?.email_address || "Unknown"} • ${
            result.student?.partner_id || id
          }`,
        });
        ...
        <div className="w-full h-full bg-[#013d7b]/90 rounded-xl relative overflow-hidden crt-screen">
          <div className="inset-0 crt-content-bulge">
            ...
            <div ref={typingRef} className="absolute inset-8">
              <TypingScreen
                isActive={appState === AppState.Typing}
                studentId={studentId}
              />
            </div>
            ...
            <div ref={displayRef} className="absolute inset-8">
              <DisplayScreen
                isActive={appState === AppState.Display}
